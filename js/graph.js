...
window.addEventListener('DOMContentLoaded', () => {
    const canvas = document.querySelector('.statspage #graph');
    if (!canvas) {
      console.error('Canvas with ID "graph" not found in the .statspage element');
      return;
    }
  
    // Set canvas dimensions explicitly based on parent container
    const parentWidth = canvas.parentElement.clientWidth;
    canvas.width = parentWidth ? parentWidth : 800;
    canvas.height = 400;
    ...
    function updateGraph() {
      const newY = Math.random() * (graphHeight - 10) + 10; // Generate a random Y value
